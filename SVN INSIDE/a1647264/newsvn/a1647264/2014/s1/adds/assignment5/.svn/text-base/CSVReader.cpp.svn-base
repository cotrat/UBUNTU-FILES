...
void CSVReader::compare(vector<Student> moodle,vector<Student> official)
{
	...
	bool written = 0;
	vector<int> outputs;
	// Simple exhaustive search to check for differences between the two CSV's
	...
	//Second Part Checking if they are in moodle but not official, reset variables
	check = 0;
	outputs.clear();
...
void CSVReader::parsePracmarkFormat()
{
	...
	ifstream  classFile("pracmarker.csv");
	string line;

	getline(classFile, line); // Skipping the first line of the file (it only contains headers)
	...
	for(unsigned int i = 0; i < line_storage.size(); i++) // The purpose of this code block is to split each line by the string delimeter and store into a vector
	{
		istringstream ss(line_storage[i]);
		string token;
		while(!ss.eof()) // Not necessarily the best conditional statement but helps to cover entries which end in a comma
		{
			getline(ss, token, ',');
		...
		if(classmems.size() >= 1)
		{
			for(x1 = 0; x1 < classmems.size(); x1++)
			{
				if(curr_id == classmems[x1].getID())
				{
					...
					/* The block of code below checks if the mark for a certain aspect is greater than the previous mark, and if so it updates
						it, atoi is used to convert strings into integers */
				...
				/* If the student is never found, we simply create a new one (this is only done once for each student) */
				if(pos < 0)
				{
					Student newStud;
...
vector<Student> CSVReader::parseOfficialFormat()
{
	...
	vector<string> split_entries;
	vector<Student> classmems;
	ifstream  classFile("officiallist.csv");
	string line;
...
vector<Student> CSVReader::parseMoodleFormat()
{
	...
	vector<Student> classmems;
	ifstream  classFile("moodlelist.csv");
	string line;
	getline(classFile, line); // Skipping the first line of the file (it only contains headers)
