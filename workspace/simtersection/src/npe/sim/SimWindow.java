...
public class SimWindow extends JFrame {
	...
	public SimWindow()
	{
		...
		if(Main.test)
		{
			pSim.start(); // Mitchell Anderson : Force the sim to start when it is created
			
			
			
			if(Main.testVis) // Mitchell Anderson: Do they want to run the test with visibility enabled
			{
				setVisible(true);	
				speedChanged(30);
			}
			...
			{
				setVisible(false);
				speedChanged(1000); // Mitchell Anderson : Force the sim to run at a high speed to ensure fast testing
			}
		}
		else
		{
