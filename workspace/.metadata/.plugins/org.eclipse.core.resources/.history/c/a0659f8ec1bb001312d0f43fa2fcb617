...
public class PedestrianLane extends Lane {
	...
	/** Mitchell Anderson: Check if a pedestrian stopped at the taxiRank **/
	...
	public void setStoppingArea(double sx, double sy)
	{
		...
		if(Main.test)
		{
		System.out.print("TEST 1 - A stopping area was created at the taxiRank\n");
		}
		...
		for (int i = 0; i < pedestrians.size(); i++) {
			...
			if (p != null) 
			{
				...
				if(p.isInArea(stoppingArea2))
				{
					...
					if(testStop == false && Main.test)
					{
						System.out.print("TEST 3 - A Pedestrian stopped at the taxi rank \n");
						testStop = true;
					}
